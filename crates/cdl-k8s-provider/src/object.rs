...
use cdl_k8s_core::openapi::{Capacity, Url};
use cdl_openapi::{
...
impl<'model> ObjectStorageSession {
    ...
    async fn create_or_get_storage(
        ...
        let cluster_role_name = format!("cdl:{tenant_name}");
        let ingress_class_controller = format!("k8s.io/cdl/{tenant_name}/{namespace}");
        ...
        labels.insert(
            "cdl.ulagbulag.io/modelstorage-type".into(),
            tenant_name.into(),
        );
        ...
        let labels_service_metrics = {
            ...
            labels.insert(
                "cdl.ulagbulag.io/modelstorage-service".into(),
                "metrics".into(),
            );
            labels
        };
    ...
    async fn create_or_get_minio_storage(
        ...
        let labels = btreemap! {
            "app".into() => "minio".into(),
            "cdl.ulagbulag.io/modelstorage-name".into() => name.into(),
            "cdl.ulagbulag.io/modelstorage-type".into() => tenant_name.into(),
...
impl<'client, 'model, 'source> ObjectStorageRef<'client, 'model, 'source> {
    ...
    async fn create_bucket_service(&self, owner_references: Vec<OwnerReference>) -> Result<()> {
        ...
        let labels = btreemap! {
            "cdl.ulagbulag.io/model-name".into() => bucket_name.clone(),
            "cdl.ulagbulag.io/modelstorage-name".into() => self.target.name.clone(),
            "cdl.ulagbulag.io/modelstorage-type".into() => tenant_name.into(),
        };
        let metadata = ObjectMeta {
            name: Some(name.clone()),
    ...
    async fn get_or_create_bucket_job(
        ...
        get_or_create(&api, "job", &name, || {
            ...
            let tenant_name = get_default_tenant_name();
            let labels = btreemap! {
                "cdl.ulagbulag.io/modelstorage-name".into() => bucket.into(),
                "cdl.ulagbulag.io/modelstorage-objectstorage-command".into() => command.into(),
                "cdl.ulagbulag.io/modelstorage-type".into() => tenant_name.into(),
            };
    ...
    K: Clone + fmt::Debug + Serialize + DeserializeOwned,
{
    match api.get_opt(name).await {
        ...
        Ok(None) => {
            let pp = PostParams {
                dry_run: false,
                field_manager: Some(crate::consts::NAME.into()),
            };
            api.create(&pp, &data()?)
                .await
            ...
            pod_affinity_term: PodAffinityTerm {
                label_selector: Some(LabelSelector {
                    match_expressions: Some(vec![LabelSelectorRequirement {
                        key: "cdl.ulagbulag.io/modelstorage-type".into(),
                        operator: "In".into(),
...
fn get_ingress_class_name(namespace: &str, tenant_name: &str) -> String {
    format!("cdl.{tenant_name}.{namespace}")
}
